...
bool can_eat_ghost(bool power_pellet_active, bool touching_ghost) {
    ...
    if(true==(power_pellet_active && touching_ghost)){
            return true;
    }
    else{
        return false;
...
bool scored(bool touching_power_pellet, bool touching_dot) {
    ...
    if(true==(touching_power_pellet || touching_dot)){
            return true;
    }
    else{
        return false;
...
bool lost = touching_ghost && !power_pellet_active;
    return has_eaten_all_dots && !lost;
}
